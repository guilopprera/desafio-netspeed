    ...
    public class ChamadosController : Controller
    {
        ...
        public IActionResult Gravar([FromBody] ChamadoRequest request)
        {
            try
            {
                ...
                if (request.dataAbertura < DateTime.Today)
                {
                    return BadRequest("Não é permitido criar chamados com data retroativa.");
                }

                if (request.idDepartamento == 0)
                {
                    return BadRequest("Por favor selecione um departamento.");
                }
